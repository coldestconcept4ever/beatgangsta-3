...
code = code.replace(/className="flex flex-col gap-2 p-3 rounded-xl bg-blue-500\/10 border border-blue-500\/20"/g, `className="absolute top-10 left-1/2 -translate-x-1/2 w-64 z-[100] flex flex-col gap-2 p-3 rounded-xl bg-blue-900 border border-blue-500/50 shadow-2xl"`);
...
code = code.replace(/(<button \s*onClick={\(C) => setIsLyricToolExpanded[\s\S]*?className={`shrink-0\s+)px-8\s+py-4(\s+rounded-full\s+font-black\s+text-)xs(\s+uppercase tracking-widest transition-all shadow-xl[\s\S]*?`}\s*>[\s\S]*?<\/button>)/, "$1px-3 py-1.5$2[10px] uppercase tracking-widest transition-all shadow-md hover:scale-105 active:scale-95 flex items-center gap-1.5 ${isLyricToolExpanded ? 'bg-sky-950 text-sky-400 border border-sky-400/50' : theme === 'coldest' ? 'bg-sky-100 text-sky-800 hover:bg-sky-200 border border-sky-200' : 'bg-sky-500/20 text-sky-300 hover:bg-sky-500/30 border border-sky-500/30'}`}> <Mic className=\"w-3 h-3\" /> {t('lyric_tool', 'Lyric Tool')} </button>");

code = code.replace(/(<button \s*onClick={\(C) => onSave\(critique\)}[\s\S]*?className={`shrink-0\s+)px-8\s+py-4(\s+rounded-full\s+font-black\s+text-)xs(\s+uppercase tracking-widest transition-all shadow-xl[\s\S]*?`}\s*>[\s\S]*?<\/button>)/, "$1px-3 py-1.5$2[10px] uppercase tracking-widest transition-all shadow-md hover:scale-105 active:scale-95 ${isSaved ? 'bg-black/10 text-current opacity-50 shadow-none' : theme === 'coldest' ? 'bg-gradient-to-b from-sky-400 to-sky-500 text-white shadow-[0_4px_15px_rgba(2,132,199,0.4)] border border-sky-400' : 'bg-white text-black'}`}> {isSaved ? t('save_to_vault') : t('save_critique')} </button>");
